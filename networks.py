...
import abc
import numpy as np # 2024.05.31 수정
    ...
    def update_normalization(self, buffer: AgentBuffer) -> None:
        ...
        for vec_input, enc in zip(obs, self.processors):
            if isinstance(enc, VectorInput):
                # enc.update_normalization(torch.as_tensor(vec_input)) # 원본 코드
                enc.update_normalization(torch.as_tensor(np.array(vec_input))) # 2024.05.31 수정
