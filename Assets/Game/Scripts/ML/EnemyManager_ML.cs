...
public class EnemyManager_ML : MonoBehaviour
{
    // Enemy들을 담을 리스트
    [SerializeField]
    [ReadOnly]
    List<Enemy_ML> enemies = new List<Enemy_ML>();
    ...
    public delegate void OnEnemiesChanged(List<Enemy_ML> enemies);
    public OnEnemiesChanged onEnemiesChanged;

    // Enemy가 죽었을 때 GameManager에게 알려주기 위한 delegate
    public delegate void OnEnemyKilled(Enemy_ML killedEnemy);
    ...
    public void SetEnemyInfo(Enemy_ML select, Player_ML player_ML, int index)
    {
        enemy_ML = select;

        enemy_ML.index = index;

        enemy_ML.name = EnemyNames[index] + enemyNameIndex; // 적들 이름 구분짓기 위한 방법
        enemyNameIndex++;

        enemy_ML.player_ML = player_ML;
        enemy_ML.onEnemyWasKilled = OnEnemyWasKilled;

        enemies.Add(enemy_ML);

        onEnemiesChanged(enemies); // GameManager에게 emeies를 전달
    }

    // Enemy가 죽었을 때 실행할 것들
    private void OnEnemyWasKilled(Enemy_ML killedEnemy, bool isKilledByPlayer)
    {
        if (isKilledByPlayer)
        {
